...
def train():
    ...
    for i in tqdm(range(
        ...
        if i % config.num_batches_validate == 0:
            (model if model_name != ' ' else '').eval()
            val_auc, val_mrr, val_ndcg5, val_ndcg10 = evaluate(
                model if model_name != ' ' else '', './data/val',
                config.num_workers, 200000)
            (model if model_name != ' ' else '').train()
            ...
            if early_stop:
                ...
                try:
                    torch.save(
                        {
                            'model_state_dict': (model if model_name != ' ' else '').state_dict(),
                            'optimizer_state_dict':
                            (optimizer if model_name != ' ' else '').state_dict(),
                            'step':
                            step,
                            'early_stop_value':
                            -val_auc
